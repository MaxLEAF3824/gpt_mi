...
def evaluate(model_name,
    ...
    if set(u_metrics) & {'se', 'sar'}:
        ...
        get_sampled_answer_prob = partial(get_sampled_answer_prob, model=model)
    ...
    if "pe" in u_metrics:
        ...
        get_uncertainty_score_token_pe_all = partial(get_uncertainty_score_token_pe_all, model=model)
        test_dst = test_dst.map(get_uncertainty_score_token_pe_all, batched=True, batch_size=eval_batch_size, new_fingerprint=str(time()))
    ...
    if "se" in u_metrics:
        ...
        get_uncertainty_score_se = partial(get_uncertainty_score_se, nli_pipe=nli_pipe, model=model)
        test_dst = test_dst.map(get_uncertainty_score_se, new_fingerprint=str(time()))
